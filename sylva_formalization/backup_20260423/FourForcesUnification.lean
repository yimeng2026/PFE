...
lemma precedes_irrefl (G : CausalNetwork) (v : CausalNode) :
  ¬G.precedes v v := by
  intro h
  rcases h with ⟨e, he, hsrc, htgt⟩
  have : e.source = e.target := by rw [hsrc, htgt]
...
def powerLawDegreeDist (γ k : ℝ) (hγ : γ > 2 ∧ γ < 3) (hk : k > 0) : ℝ :=
  k ^ (-γ)
...
noncomputable def standardTunneling (T : InterLayerTransition) : ℝ :=
  tunnelingFactorFormula (T.layerDistance.toFloat : ℝ)
...
def spatialConnectivity (G : CausalNetwork) (v : CausalNode) (timeSlice : CausalNode → ℝ) : ℝ :=
...
/-- Charge quantization theorem:
    Charge Q corresponds to H²(G, ℤ), hence automatically quantized -/
...
/-- Black hole entropy from surface node counting:
    S_BH = A / (4Gℏ) emerges from network boundary nodes -/
