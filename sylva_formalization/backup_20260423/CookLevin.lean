    ...
    | CircuitNode.gate gt l r => 
        have hl : l < idx := by
          have h_gate : C.numInputs ≤ idx := by
            ...
            have h_node_input : C.nodes.get ⟨idx, h⟩ = CircuitNode.input idx :=
              C.hwf.input_spec idx h_input h
            rw [h_node_input] at heq
            injection heq
          rcases C.hwf.gate_spec idx h_gate h with ⟨gt', l', r', h_eq', hl', hr'⟩
          rw [heq] at h_eq'
...
lemma evalNode_gate_eq (C : BooleanCircuit) (state : List Bool) (idx : ℕ)
  ...
  -- Use the definition of evalNode directly with the gate case
  have h_def : evalNode C state idx = 
    evalGate gt (evalNode C state l) (evalNode C state r) := by
    -- Apply the equation lemma for evalNode
    rw [evalNode]
    simp [hidx, heq]
    -- The well-founded recursion should reduce to the gate case
    -- since we know the node at idx is a gate
    all_goals try { simp [WellFounded.fix_eq] }
    all_goals try { rfl }
    all_goals try { simp [evalGate] }
  exact h_def
