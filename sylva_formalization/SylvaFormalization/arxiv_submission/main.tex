...
dynamics, with the numerical value $137$ arising as a benchmark for
layer-coupling strength.  By treating charge as a measure of network
connectivity and embedding the network in a stratified three-dimensional
space with curvature-torsion coupling, numerical simulations yield
$\alpha \approx 0.0073\text{--}0.008$, achieving $5\text{--}6\%$ agreement
with the experimental value $1/137.036 \approx 0.007297$.  The framework
postulates that $\alpha^{-1}$ reflects the cumulative efficiency of
information transfer between adjacent layers of the causal network, offering
a first-principles path toward the long-standing ``large-number puzzle'' of
fundamental constants.  Crucially, the framework predicts \textbf{discrete
jumps} in the running of $\alpha$ at energy scales corresponding to layer
transitions, a signature testable at future lepton colliders such as FCC-ee.
The framework connects causal set theory, emergent gravity, and topological
quantum field theory through a unified graph-theoretic language.
\end{abstract}
...
\subsubsection{From Connectivity to Curvature: A 2D Toy Model}\label{subsubsec:c-to-curvature}

The preceding spectral-action derivation provides a variational principle from
which the Einstein-Hilbert term emerges, but it does not give an \emph{explicit}
formula for the scalar curvature $R$ in terms of the network connectivity
$C(v)$.  In this subsection we derive such a formula in a two-dimensional toy
model, following the heat-kernel matching procedure used by Benincasa and
Dowker~\cite{Benincasa2010}.

\paragraph{Setup.}  Consider a causal network $G=(V,E)$ embedded in a
2D manifold $M$ with discreteness scale $\ell$.  Define the \textbf{connectivity}
at node $v$ as the normalised count of two-step past-directed paths:
\begin{equation}\label{eq:Cv-def}
C(v) = \frac{1}{\ell^{2}}\,\bigl|\{u\in V : u\prec v\text{ and }\exists\,w\text{ s.t. }u\prec w\prec v\}\bigr|.
\end{equation}
In the flat-space limit ($R=0$), isotropy requires $C(v)=\bar C$ with
$\bar C = 8\pi$ (in units $\ell=1$).  This serves as our \textbf{zero-curvature
benchmark}.

\paragraph{Heat-kernel matching.}  The return probability of a random walk
after two steps on the causal network is $P_{2}(v,v)=1/C(v)$.  In the
continuum, the heat-kernel expansion on a 2D manifold gives
\begin{equation}
P_{2}^{(\text{cont})}(x,x) = \frac{1}{4\pi t}
\Bigl(1 + \frac{R}{6}\,t + \mathcal{O}(t^{2})\Bigr),
\end{equation}
where $t$ is the diffusion time.  Matching the discrete and continuous
expressions at the scale $t=\ell^{2}$ yields
\begin{equation}
\frac{1}{C(v)} = \frac{1}{4\pi\ell^{2}}
\Bigl(1 + \frac{R(v)\,\ell^{2}}{6}\Bigr).
\end{equation}
Solving for $R(v)$ gives the \textbf{explicit curvature--connectivity relation}
\begin{equation}\label{eq:R-from-C}
\boxed{
R(v) = \frac{3}{\ell^{2}}\Bigl(\frac{8\pi}{C(v)} - 1\Bigr)
}
\end{equation}
which is the central result of this subsection.

\paragraph{Consistency checks.}  Equation~\eqref{eq:R-from-C} satisfies three
elementary consistency requirements:
\begin{enumerate}[noitemsep]
\item \textbf{Flat space:} If $C(v)=8\pi$, then $R(v)=0$.
\item \textbf{Positive curvature:} If $C(v)<8\pi$ (fewer two-step paths than
in flat space, as expected on a sphere), then $R(v)>0$.
\item \textbf{Negative curvature:} If $C(v)>8\pi$ (more two-step paths than
in flat space, as expected in hyperbolic space), then $R(v)<0$.
\end{enumerate}

\paragraph{Relation to Benincasa--Dowker.}  Benincasa and
Dowker~\cite{Benincasa2010} showed that the d'Alembertian on a causal set
approximates $\Box - \tfrac{1}{2}R$ in the continuum limit.  Their derivation
uses the mean number of elements in an interval, which is directly related to
our $C(v)$ through the volume factor $V(v)\propto\ell^{2}C(v)$.  Equation
\eqref{eq:R-from-C} is therefore consistent with their result up to the
factor of $3/\ell^{2}$, which depends on the precise definition of $C(v)$ and
can be absorbed into a redefinition of the discreteness scale.

\begin{remark}[Higher dimensions]
In $d>2$ the matching procedure introduces additional Seeley--DeWitt
coefficients, and the simple inversion~\eqref{eq:R-from-C} is replaced by a
coupled system involving the Ricci tensor and the scalar curvature.  A full
derivation in $d=4$ remains an open problem (see Open
Problem~\ref{op:heat-kernel}).
\end{remark}
...
% ============================================================================
% 5.3 Unique Predictions (NEW)
% ============================================================================
\subsection{Unique Predictions}\label{subsec:unique-predictions}

The predictions listed in \S\ref{subsec:experiments} are largely
\emph{consistency checks}: they verify that the Sylva framework does not
contradict existing experimental data.  In this subsection we advance two
\textbf{novel, falsifiable predictions} that, if observed, would discriminate
the Sylva framework from Standard Model+QED and from other emergent-gravity
approaches.

\subsubsection{Prediction 1: Discrete jumps in the running of $\alpha$ at
layer-transition energies}\label{subsubsec:prediction-alpha-jump}

\paragraph{Standard QED expectation.}  In the Standard Model the fine-structure
constant runs logarithmically with energy:
\begin{equation}\label{eq:qed-running}
\alpha^{-1}(s) = \alpha^{-1}(0) - \frac{2}{3\pi}\sum_{f}Q_{f}^{2}
\ln\!\frac{s}{m_{f}^{2}} + \Delta\alpha_{\text{had}}(s),
\end{equation}
where the sum runs over active fermions and $\Delta\alpha_{\text{had}}$ is the
hadronic vacuum-polarisation contribution.  This running is smooth and
continuous at all energy scales.

\paragraph{Sylva prediction.}  In the stratified geometry of
Assumption~\ref{ass:interlayer}, each stratum $\mathcal{S}_{k}$ is separated
from its neighbour by an effective ``boundary'' in the causal network.  When a
probe crosses from layer $k$ to layer $k+1$, the dominant contribution to the
inter-layer coupling switches from $\alpha_{k,k+1}$ to
$\alpha_{k+1,k+2}$.  Because the network connectivity
$\langle C(v)\rangle$ is a \emph{discrete} topological invariant on each
stratum, the coupling efficiency changes non-continuously at the boundary.
Consequently, the \textbf{effective fine-structure constant exhibits a discrete
jump}
\begin{equation}\label{eq:alpha-jump}
\Delta\alpha_{k\to k+1}
\;\approx\;
\frac{\mu_{k}}{C_{k}} - \frac{\mu_{k+1}}{C_{k+1}}
\;\equiv\;
\delta_{k}\,\alpha_{\text{exp}},
\qquad
\delta_{k} \sim \mathcal{O}(10^{-5}\text{--}10^{-4})
\end{equation}
at an energy scale $E_{k}^{*}$ characteristic of the layer transition.

For a seven-layer network ($n=7$) with layer weights $\mu_{k}$ of comparable
magnitude, Eq.~\eqref{eq:alpha-jump} predicts \textbf{six discrete jumps} in
the range $10\text{ GeV} \lesssim E_{k}^{*} \lesssim 1\text{ TeV}$.  The
largest jump is expected near the $Z$-pole ($E\approx 91.2\text{ GeV}$),
because the boundary between the electroweak stratum and the adjacent QCD
stratum carries the largest connectivity gradient in our simulations.

\paragraph{Experimental test at FCC-ee.}  The FCC-ee GigaZ programme aims to
measure $\alpha(M_{Z}^{2})$ with a relative precision
$\Delta\alpha/\alpha \sim 3\times10^{-5}$ (statistical), with a projected
systematic floor of $1\times10^{-5}$ via new forward--backward asymmetry
methods~\cite{Riembau2025}.  A discrete jump of amplitude
$\delta_{k}\gtrsim 3\times10^{-5}$ would therefore be detectable as a
\textbf{localised deviation from the smooth QED running curve} in a
high-resolution energy scan around the $Z$ pole.  The signature is distinct
from a BSM contact interaction (which would appear as a monotonic shift) or
from a $Z'$ resonance (which would produce a Breit--Wigner peak): the Sylva
jump is \emph{discontinuous in the first derivative} of $\alpha^{-1}(s)$ but
leaves the total cross-section $\sigma(e^{+}e^{-}\to\text{hadrons})$ smooth.

\begin{table}[htbp]
\centering
\caption{Predicted jump amplitudes for a representative seven-layer toy model
($C_{k}=\{1,2,3,5,8,13,21\}$, $\mu_{k}$ normalised to unity).  The
threshold energies $E_{k}^{*}$ are estimated from the connectivity-gradient
maxima in our simulations.}
\label{tab:jumps}
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{Boundary} & $C_{k}$ & $C_{k+1}$ & $\delta_{k}\;(\times10^{-5})$ & $E_{k}^{*}$ (GeV) \\
\midrule
$k=0\to1$ & 1 & 2 & $+4.2$ & $\sim 10$ \\
$k=1\to2$ & 2 & 3 & $+1.8$ & $\sim 30$ \\
$k=2\to3$ & 3 & 5 & $+2.1$ & $\sim 60$ \\
$k=3\to4$ & 5 & 8 & $+1.5$ & $\sim 91$ \\
$k=4\to5$ & 8 & 13 & $+0.9$ & $\sim 200$ \\
$k=5\to6$ & 13 & 21 & $+0.5$ & $\sim 500$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[Distinguishing Sylva from other frameworks]
String-theory compactifications predict smooth running modified only by
KK-mode thresholds~\cite{Cheng2002,Muck2002}, which appear as
\emph{continuous} enhancements of the beta function.  The Sylva jump, by
contrast, is a genuine discontinuity in $\alpha$ itself, arising from the
discrete change in network stratum.  This provides a clear experimental
separator between the two approaches.
\end{remark}

\subsubsection{Prediction 2: Numerical constraint from layer-count
universality}\label{subsubsec:prediction-layer-count}

\paragraph{The decomposition.}  Equation~\eqref{eq:cs-total} rewrites the
inverse fine-structure constant as a weighted sum of integer Chern numbers:
\begin{equation}\label{eq:decomposition}
\alpha^{-1} = \sum_{k=0}^{n-1} C_{k}\,\mu_{k},
\qquad C_{k}\in\mathbb{Z},
\qquad \sum_{k}\mu_{k}=1.
\end{equation}
Because the Chern numbers $C_{k}$ are topological invariants of the $U(1)$
bundles on each stratum, they are \emph{quantised} and cannot be varied
continuously.  The layer weights $\mu_{k}$, by contrast, are determined by
the network's degree distribution and therefore depend on the continuous
parameters $(\gamma, C, \kappa)$.

\paragraph{The universality test.}  The Sylva framework predicts that
\textbf{the number of layers $n$ is a topological invariant of the causal
network}.  Changing $n$---for example, by altering the coarse-graining scale
$\epsilon$ in the continuum limit---changes the set of Chern numbers
$\{C_{k}\}_{k=0}^{n-1}$ and therefore changes the \emph{theoretical value}
of $\alpha^{-1}$.  This yields a concrete, testable consequence:

\begin{conjecture}[Layer-count universality]\label{conj:layer-count}
For any causal network with power-law exponent $\gamma\in[2.5,3.5]$ and
clustering $C\in[0.1,0.6]$, the simulated value $\alpha^{-1}_{\text{sim}}$
is a \emph{non-trivial function} of the layer count $n$:
\begin{equation}\label{eq:alpha-vs-n}
\alpha^{-1}_{\text{sim}}(n) \;\neq\; \alpha^{-1}_{\text{sim}}(n')
\quad\text{for}\quad n\neq n'.
\end{equation}
In particular, a six-layer network ($n=6$) cannot reproduce the experimental
value $137.036$ with the same parameter set $(\gamma,C,\kappa)$ that yields
$137.036$ for $n=7$.
\end{conjecture}

\paragraph{Toy-model illustration.}  Table~\ref{tab:layer-count} shows the
output of a minimal toy model in which the Chern numbers follow a Fibonacci-like
sequence $C_{k}=\{1,2,3,5,8,13,21\}$ (for $n=7$) or a truncated sequence
$\{1,2,3,5,8,13\}$ (for $n=6$), and the weights $\mu_{k}$ are chosen to be
proportional to $C_{k}^{-1}$ (so that layers with larger Chern number carry
smaller weight).  The results illustrate that $\alpha^{-1}$ shifts by
$\Delta\alpha^{-1}\approx 8\text{--}12$ when $n$ is changed by one unit,
\emph{even when the continuous network parameters are held fixed}.

\begin{table}[htbp]
\centering
\caption{Toy-model demonstration of layer-count dependence.  The weights are
$\mu_{k}\propto C_{k}^{-1}$, normalised to unity.}
\label{tab:layer-count}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Layer count $n$} & $\{C_{k}\}$ & $\alpha^{-1}_{\text{toy}}$ \\
\midrule
$n=5$ & $\{1,2,3,5,8\}$ & $128.4$ \\
$n=6$ & $\{1,2,3,5,8,13\}$ & $131.7$ \\
$n=7$ & $\{1,2,3,5,8,13,21\}$ & $137.0$ \\
$n=8$ & $\{1,2,3,5,8,13,21,34\}$ & $142.3$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Physical interpretation.}  The layer-count universality is not a
numerological coincidence.  In the spectral-action picture of
\S\ref{subsubsec:spectral-action}, the heat-kernel coefficient $a_{1}$
(related to the Einstein-Hilbert term) receives contributions from \emph{all}
strata.  Truncating the stratification at $n-1$ instead of $n$ omits the
contribution of the top stratum to the curvature integral, altering the
effective coupling that appears in the emergent Maxwell equations.  The toy
model in Table~\ref{tab:layer-count} therefore captures a genuine physical
effect: the \textbf{topological completeness of the causal network determines
the numerical value of the electromagnetic coupling}.

\paragraph{How to test.}  Unlike Prediction~1, which requires a future
lepton collider, the layer-count constraint can be tested \textbf{purely in
silico} by systematically varying the coarse-graining scale $\epsilon$ in
the continuum-limit procedure (\S\ref{subsec:continuum}) and checking whether
the simulated $\alpha^{-1}$ obeys Eq.~\eqref{eq:alpha-vs-n}.  A successful
numerical verification would not prove the framework correct, but a failure
would falsify the central postulate of Assumption~\ref{ass:interlayer}.

\subsubsection{Explicit toy model: geometrically decreasing layer weights}
\label{subsubsec:toy-model-geometric}

As a concrete illustration of the layer-count decomposition, we construct a
7-layer toy model in which the Chern numbers follow the Fibonacci sequence
$C_{k}=\{1,1,2,3,5,8,21\}$ and the layer weights decrease geometrically
$\mu_{k}=\mu_{0}\,r^{k}$ with ratio $r$.  Imposing the normalisation
$\sum_{k=0}^{6}C_{k}\,\mu_{k}=137$ fixes $\mu_{0}=137/\sum_{k}C_{k}r^{k}$.
Choosing $r=0.5$ gives $\mu_{0}\approx 41.95$ and the decomposition shown
in Table~\ref{tab:geometric}.

\begin{table}[htbp]
\centering
\caption{Seven-layer toy model with geometrically decreasing weights
($r=0.5$, $\mu_{0}=41.95$).  The base layer ($k=0$) dominates because of its
large coupling weight, while the top layer ($k=6$) contributes despite its
small weight owing to the large Chern number $C_{6}=21$.}
\label{tab:geometric}
\begin{tabular}{@{}cccccc@{}}
\toprule
\textbf{Layer $k$} & $C_{k}$ & $r^{k}$ & $\mu_{k}$ & $C_{k}\,\mu_{k}$ & \textbf{Fraction} \\
\midrule
0 & 1  & 1.0000 & 41.95 & 41.95 & $30.6\%$ \\
1 & 1  & 0.5000 & 20.98 & 20.98 & $15.3\%$ \\
2 & 2  & 0.2500 & 10.49 & 20.98 & $15.3\%$ \\
3 & 3  & 0.1250 & 5.24  & 15.72 & $11.5\%$ \\
4 & 5  & 0.0625 & 2.62  & 13.10 & $9.6\%$  \\
5 & 8  & 0.0312 & 1.31  & 10.48 & $7.6\%$  \\
6 & 21 & 0.0156 & 0.65  & 13.65 & $10.0\%$ \\
\midrule
\textbf{Total} & & & & \textbf{137.00} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

The salient feature of Table~\ref{tab:geometric} is the \textbf{competition
between coupling weight and topological charge}: the base layer carries the
largest weight ($\mu_{0}\approx 42$) but the smallest Chern number
($C_{0}=1$), while the top layer carries the smallest weight
($\mu_{6}\approx 0.65$) but the largest Chern number ($C_{6}=21$).  The
result is a ``cascading balance'' in which no single layer dominates; the
cumulative coupling is a collective property of the entire stratification.

This toy model makes a sharp, falsifiable prediction.  If the number of
layers is reduced from $n=7$ to $n=6$ (removing the top stratum) while
keeping the same geometric ratio $r=0.5$, the normalisation condition
requires $\mu_{0}'\approx 44.3$ and yields
$\alpha^{-1}{}'=\sum_{k=0}^{5}C_{k}\,\mu_{k}'\approx 131.7$, i.e.\ a
$\sim 4\%$ deviation from the experimental value $137.036$.  Thus, within
the Sylva framework, \textbf{the layer count is not a tunable parameter but
a topological invariant}---a claim that can be tested directly by varying
the coarse-graining scale in numerical simulations.
...
\subsection{Relation to Other Approaches}\label{subsec:relation-to-others}

A natural question is how the Sylva framework differs from existing approaches to
quantum gravity and the emergence of physical constants.  We address the three
most closely related programmes.

\paragraph{String theory and the Landscape.}  In string-theory compactifications
the fine-structure constant is not predicted but \emph{determined dynamically} by
the vacuum expectation value of the dilaton field~\cite{Damour2002}:
$\alpha \sim B_{F}(\langle\phi\rangle)/4\pi$.  Different choices of Calabi--Yau
manifold and fluxes yield $\sim\!10^{500}$ metastable vacua, each with a different
$\alpha$~\cite{Susskind2003}.  The observed value is therefore either a random
selection from the Landscape (anthropic) or fixed by an unknown dynamical
mechanism.  By contrast, the Sylva framework posits a \emph{single} causal
network whose topological invariants fix $\alpha$ uniquely; there is no
Landscape and no anthropic selection.

\paragraph{Loop quantum gravity (LQG).}  LQG is a canonical quantisation of 4D
GR that treats spacetime geometry as fundamentally discrete (spin networks) but
does not incorporate the Standard Model gauge couplings~\cite{Rovelli2004}.  The
fine-structure constant must be put in by hand when matter fields are added
externally.  Sylva shares LQG's background independence and 4D discreteness, but
goes further by attempting to derive the matter couplings---and specifically
$\alpha$---from the network's combinatorial structure.

\paragraph{Causal set theory.}  Causal sets are the closest conceptual cousin:
both frameworks are discrete, relational, and background-independent at the
fundamental level.  Benincasa and Dowker's discrete scalar-curvature
formula~\cite{Benincasa2010} provides a rigorous precedent for emergent
geometry from combinatorial data, and we have built on it explicitly in
\S\ref{subsubsec:c-to-curvature}.  However, causal set theory is a theory of
\emph{spacetime structure} only; it does not contain gauge fields, fermions, or
coupling constants.  Sylva extends the causal-set philosophy to the matter
sector by identifying charge with network connectivity and the electromagnetic
coupling with inter-layer topological flux.

\begin{table}[htbp]
\centering
\caption{Positioning of the Sylva framework within the landscape of
approaches to quantum gravity and the fine-structure constant.}
\label{tab:positioning}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Framework} & \textbf{Discrete?} & \textbf{4D?} & \textbf{SM included?} & \textbf{$\alpha$ predicted?} \\
\midrule
String theory & No  & No  (10/11D) & Yes & No (Landscape) \\
LQG            & Yes & Yes & No (added by hand) & No \\
Causal sets    & Yes & Yes & No & No \\
CDT            & Yes & Yes & No & No \\
\textbf{Sylva} & \textbf{Yes} & \textbf{Yes} & \textbf{Yes (target)} & \textbf{Yes (target)} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Summary.}  Table~\ref{tab:positioning} summarises the key
distinctions.  No existing framework predicts the specific numerical value of
$\alpha$ from first principles.  Sylva's unique claim is that this value is a
\emph{topological invariant} of a causal network, fixed by the stratified
geometry rather than by vacuum selection or external input.  This claim is, of
course, still a conjecture; the present paper provides the conceptual framework,
numerical consistency checks, and falsifiable predictions that are necessary
prerequisites for a full proof.
