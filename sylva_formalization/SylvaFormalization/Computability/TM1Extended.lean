...
postulate stepN_ReachesIn (M : Machine Γ Λ σ n_tapes) (k : ℕ) (c : Config Γ Λ σ n_tapes) :
...
postulate accepts_in_iff_Reaches (M : Machine Γ Λ σ n_tapes) (k : ℕ) (input : List Γ) :
...
postulate TM1Compat_roundtrip (c : Turing.TM1.Cfg Γ Λ σ) :
...
postulate TM1toMultitapeMachine (M : Λ → Turing.TM1.Stmt Γ Λ σ) :
...
postulate TM1Compat_step (M : Λ → Turing.TM1.Stmt Γ Λ σ) (c : Turing.TM1.Cfg Γ Λ σ) :
...
postulate multitape_to_singletape_overhead
...
postulate singletape_to_multitape_linear
    (M : Turing.TM0.Machine Γ Λ) :
